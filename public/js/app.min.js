...
var modal = '<div id="modal_delete' + id + '">';
    ...
    modal +=              '<button id="close-mdl-delete-'+ id +'" type="button" class="close" data-dismiss="modal" aria-label="Close">';
    modal +=                '<i class="ti-close"></i>';
    modal +=              '</button>';
    modal +=           '</div>';
    modal +=           '<div class="modal-body">';
    modal +=             'Are you sure want to delete <b>' + title + '</b> ?';
    modal +=           '</div>';
    modal +=           '<div class="modal-footer" data-url="'+url+'" data-token="'+token+'" id="data-form-delete-' + id + '">';
    //modal +=            '<form method="POST" action="' + url + '" id="data-form-delete-' + id + '" role="form" accept-charset="utf-8">';
    ...
    modal +=                '<button type="button" onclick="removeFnRowDatatable('+id+')" class="btn btn-danger btn-delete">Delete</button>';
    //modal +=            '<form>';
    modal +=          '</div>';
    modal +=        '</div>';
    modal +=      '</div>';
  ...
  if(check_data == 0){
    $('#area_modal' + id).html(modal);
  }
}

function removeFnRowDatatable(id) {
  var button      = $('#data-form-delete-'+ id).find('.btn-delete');
  var buttonHtml  = button.html();
  var getForm     = $('#data-form-delete-'+ id);
  var action      = getForm.attr('data-url');
  var token       = getForm.attr('data-token');
  var getUrl      = action;
  $.ajax({
    type  : "GET",
    url   : action,
    ...
    success: function(data){
      $('#close-mdl-delete-' + id).click();
      button.removeClass('disabled');
      button.html(buttonHtml);
      if (data.status == 1) {
        alert_(1, data.message);
        $('#tables').DataTable().ajax.reload(null, false);
      }else{
        alert_(2, data.message);
      }
    },
    error: function(){
      $('#close-mdl-delete-' + id).click();
      button.removeClass('disabled');
      button.html(buttonHtml);
      alert_(2, 'Something wrong, try again / reload the page');
    }
  })
}
function alert_(status, text){
  var status = (status == 1)? "alert-success" : "alert-danger";
  var data_alert = '<div class="alert '+ status +' alert-dismissible" role="alert">'+
                      text +
                      '<button type="button" class="close" data-dismiss="alert" aria-label="Close">'+
                        '<i class="ti-close"></i>'+
                      '</button>'+
                    '</div>';
  $('#notif_info').html(data_alert);
  }